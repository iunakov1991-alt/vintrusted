...
async function generateArticles(count = 10000) {
  ...
  for (const article of articles) {
    try {
      ...
      const keywords = generateKeywords(article.originalSlug, title);
      ...
      const page = generatePage({
        ...
        ogTitle: `${title} - VIN Trust | Instant Vehicle History Reports`,
        ogDescription: description,
        schemaType: 'Article',
        schemaName: title,
        schemaDescription: content[0].html.replace(/<[^>]*>/g, '').substring(0, 200),
        content: htmlContent
      });
