...
// (API routes are defined later in the file, but we ensure they're registered)
...
// Service Worker - before static files
 ...
 * Получить дерево стратегии
 */
app.get('/api/strategy/tree', (req, res) => {
  try {
    const path = require('path');
    const scriptPath = path.join(ROOT_DIR, 'scripts', 'build_strategy_tree.js');
    const { buildStrategyTree } = require(scriptPath);
    ...
    console.error('[Dashboard] Error building strategy tree:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});
