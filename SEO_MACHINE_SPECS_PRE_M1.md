# 📋 СПЕЦИФИКАЦИИ SEO МАШИНЫ 6.0 (ДО M1)
...
## 🤖 AI СИСТЕМА (ТОЛЬКО API)

### Провайдеры (в порядке приоритета)
1. **Groq** - Llama 3.1 8B (быстрый, бесплатный, primary)
2. **DeepSeek** - DeepSeek Chat (fallback)
3. **OpenAI** - GPT (опционально, последний fallback)

### Параметры AI
- **Max Tokens:** 400 (оптимизировано для скорости)
- **Timeout:** 15 секунд (Groq), 25 секунд (DeepSeek)
- **Retry:** 2 попытки
- **Cache:** 9,185+ записей
- **Rate Limit Protection:** Автоматическое отключение Groq при 429
...
### Особенности
- **Groq Rate Limit:** Автоматическое отключение при 429, fallback на DeepSeek
- **Client-side Rate Limiting:** Защита от превышения лимитов
...
- ❌ **localAI** - Локальный AI (не поддерживается)
- ❌ **m1Optimization** - M1 оптимизация (не поддерживается)
...
2. ✅ **Memory Monitor** - Мониторинг памяти (отключен на Vercel)
...
- **API-based AI:** Только Groq/DeepSeek/OpenAI
- **Нет локального AI:** Только облачные провайдеры
...
- **С Groq:** ~1,500 страниц/минуту (быстрее)
- **С DeepSeek:** ~800 страниц/минуту (медленнее, но надежнее)
...
- **Rate Limit Protection:** Автоматическое переключение провайдеров

### Ограничения
- **Memory:** Защита от переполнения (85% threshold)
- **Timeout:** AI запросы с таймаутами (15s Groq, 25s DeepSeek)
- **Concurrency:** Адаптивная конкуренция (10-25 потоков)
- **Rate Limits:** Автоматический fallback между провайдерами
...
- **Fallback Mechanisms** - Резервные механизмы (Groq → DeepSeek → OpenAI)
- **Auto Repair** - Автоматическое исправление

### Мониторинг
- **Memory Monitor** - Мониторинг памяти (отключен на Vercel)
...
### API Провайдеры
- **Groq:** Бесплатный план (ограничения по rate limit)
- **DeepSeek:** Платный (но дешевле OpenAI)
- **OpenAI:** Платный (самый дорогой, последний fallback)

### Оптимизация затрат
- **Groq Priority:** Используется первым (бесплатно)
- **DeepSeek Fallback:** Только при ошибках Groq
- **OpenAI Last Resort:** Только в критических случаях
- **Кэширование:** 90% запросов из кэша (экономия)
...
- ❌ **Local AI Provider** - Нет поддержки Ollama
- ❌ **M1 Optimizer** - Нет оптимизации для MacBook M1
- ❌ **M1-specific concurrency** - Нет специальных настроек потоков
- ❌ **Local AI timeout** - Нет 60-секундных таймаутов для локального AI
...
### Критические
- ⚠️ **Groq Rate Limits:** Периодические 429 ошибки (автоматический fallback)
- ⚠️ **DeepSeek Timeout:** Иногда таймауты на длинных промптах
...
- **Совместимость:** Vercel-оптимизированная версия
...
**Версия спецификаций:** 1.0 (Pre-M1)
