...
function renderSeoPage(pageData) {
  const lang = pageData.lang || "en";
  const dir = "ltr";
  const hreflang = pageData.hreflang || {};
  
  // Provide default values for hreflang properties to prevent undefined errors
  const hreflangSelf = hreflang.self || pageData.url || pageData.canonical || '/';
  const hreflangAltLang = hreflang.altLang || hreflangSelf;
  const hreflangXDefault = hreflang.xDefault || hreflangSelf;
...
<head>
  ...
  <link rel="canonical" href="${escapeHtml(hreflangSelf)}" />
  <link rel="alternate" href="${escapeHtml(hreflangSelf)}" hreflang="${lang}" />
  <link rel="alternate" href="${escapeHtml(hreflangAltLang)}" hreflang="${lang === "en" ? "es" : "en"}" />
  <link rel="alternate" href="${escapeHtml(hreflangXDefault)}" hreflang="x-default" />
</head>`;
