  ...
  async function workerLoop() {
    while (true) {
      ...
      try {
        ...
        log('MASTER', `Error processing item ${i}: ${e.message}`);
        results[i] = null;
      }
    }
  }
...
async function main() {
  ...
  // Проверяем, был ли файл создан в этом же билде (в течение последних 30 секунд)
  if (isVercel && fs.existsSync(BUILD_META_PATH)) {
    try {
      const existingMeta = safeLoadJson(BUILD_META_PATH, {});
      if (existingMeta.finishedAt) {
        // Файл завершен - это второй проход в том же деплое
        log('MASTER', 'Second Vercel pass detected, skipping SEO build.');
        process.exit(0);
      }
      // Файл существует, но не завершен - удаляем и продолжаем (возможно, прерванный билд)
      log('MASTER', 'Found incomplete build meta, removing and continuing...');
...
main().catch((e) => {
  log('MASTER', `Fatal error: ${e.message}`);
  console.error(e);
  process.exit(1);
});
