...
async function generateAndAddArticles(count = 5000) {
  ...
  for (const article of articles) {
    try {
      ...
      // Convert content HTML to pageData format
      // Extract sections from content HTML
      const sections = [];
      const sectionMatches = content.matchAll(/<section[^>]*>\s*<h2[^>]*>(.*?)<\/h2>\s*(.*?)<\/section>/gs);
      let sectionId = 1;
      ...
      const introHtml = introMatch ? `<p>${introMatch[1]}</p>` : `<p>Learn everything about ${title} with VIN Trust's comprehensive vehicle history reports.</p>`;
      
      const pageData = {
        slugPath: `/articles2/${article.slug}`,
        title: `${title} | VIN Trust - Vehicle History Report`,
        metaDescription: `Complete guide to ${title}. Get comprehensive vehicle history reports, accident records, and title verification for just $3.`,
        h1: title,
        introHtml: introHtml,
        sections: sections.length > 0 ? sections : [
          {
            id: 'main-content',
            heading: `About ${title}`,
            html: content.replace(/<h[1-6][^>]*>.*?<\/h[1-6]>/g, '')
          }
        ],
        benefitsHtml: `<ul>
