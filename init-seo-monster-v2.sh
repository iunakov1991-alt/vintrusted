...
# SEO-MONSTER v2 — АВТОНОМНЫЙ SEO-МОНСТР ДЛЯ vintrusted.com

#

# ВАЖНО:

#   - НЕ трогает фронт, Stripe, ClearVIN, API-роуты и главную.

#   - Работает ПОВЕРХ существующего проекта, как надстройка.

#   - Все SEO-HTML лежат в public/seo/pages (отдельное дерево).

#   - sitemap-ы лежат в public/seo/sitemaps.

#   - Dashboard и build-meta лежат в public/internal.

#   - AI подключается через универсальный endpoint (DeepSeek, Grok и т.п.).

#

# VERCEL:

#   - Скрипт НЕ ломает существующий vercel-build:

#       если vercel-build уже есть:

#         станет: "node scripts/seo/seo-master-build.js && <старый>"

#       если нет:

#         станет: "node scripts/seo/seo-master-build.js"

#   - НИКАКИХ новых rewrites на /vin, /, /checkout и т.п.

#   - Все существующие rewrites/redirects в vercel.json сохраняются.
...
#     npm run seo-monster       # только SEO-монстр

#

#   Vercel:

#     npm run vercel-build      # сначала SEO-монстр, потом твой старый билд

########################################################################



echo "[SEO-MONSTER v2] Init started..."
...
# 4. seo-ai-client.js — AI с кешем, endpoint-ом и fallback
...
# 10. seo-graph-engine.js — граф (анализ, без изменения HTML)
...
# 13. seo-master-build.js — главный вход для SEO-монстра
...
# 14. setup-seo-config.js — аккуратный патч package.json и vercel.json
...
echo "[SEO-MONSTER v2] Init finished. Локально: npm run seo-monster. На Vercel: npm run vercel-build."
