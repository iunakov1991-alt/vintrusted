...
########################################################################
# SEO-MONSTER 4.0 — TWO-STAGE MONSTER (CORE 2.x + SEO 3.0) FOR VERCEL PRO
#
# ВАЖНО:
#   - НИЧЕГО не трогать во фронте, Stripe, ClearVIN и API.
#   - Работать ТОЛЬКО в:
#       scripts/seo/*
#       data/seo/*
#       public/seo/*
...
########################################
# 6. seo-ai-client.js — оптимизированный AI-клиент без fetch
########################################
...
async function generateText(prompt, { lang = 'en', intent = 'generic', maxTokens = 600 } = {}) {
  ...
  const effectiveAI = configEnableAI && envEnable && !!process.env.SEO_AI_API_KEY;
 ...
 *    ссылку на https://vintrusted.com/seo/sitemaps/sitemap-seo.xml (если её ещё нет)
 */
